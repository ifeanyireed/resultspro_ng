              ...
              <div className="flex gap-2 p-1.5 bg-white border border-slate-200/60 rounded-full shadow-sm">
                {steps.map((s) => (
                  <button
                    key={s.id}
                    disabled={step < s.id}
                    className={`px-6 py-2.5 rounded-full text-sm font-extrabold uppercase tracking-wider transition-all duration-300 flex items-center ${
                      step === s.id 
                    ...
                    <div className="space-y-8">
                      ...
                      <div>
                        ...
                        <input 
                          ...
                          onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormData({ ...formData, motto: e.target.value })}
                          className={inputStyle}
                        />
                      </div>
