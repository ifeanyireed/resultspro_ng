...
export default function MarketOpportunity() {
  const stats = [
    ...
    { value: "4th", label: "Largest Global Pop. 2050", accent: "text-white" }
  ];
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 mb-32">
          {stats.map((stat, i) => (
            <motion.div
              ...
              className="glass p-8 rounded-[2rem] border-white/5 group hover:bg-white/[0.05] transition-all duration-500"
            >
              ...
              <p className="text-gray-500 text-[11px] font-black uppercase tracking-[0.1em] font-display">{stat.label}</p>
            </motion.div>
          ))}
        </div>
        ...
        <div className="glass p-12 md:p-24 rounded-[4rem] border-white/5 relative overflow-hidden group">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-16 relative z-10">
            {phases.map((phase, i) => (
              ...
              >
                <span className="text-8xl font-black text-white/5 absolute -top-10 -left-6 font-display transition-colors group-hover/phase:text-white/10">{phase.number}</span>
                <div className="relative z-10">
                  <h3 className="text-2xl font-black text-white mb-6 font-display tracking-tight leading-none">{phase.title}</h3>
