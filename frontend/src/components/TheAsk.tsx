        ...
        <div className="flex flex-col lg:flex-row justify-between items-end gap-16 mb-32">
          <div className="lg:w-2/3">
            ...
            <motion.p 
              ...
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              className="text-gray-400 text-xl font-body leading-relaxed max-w-xl"
            >
          ...
          >
            ...
            <div className="text-white text-6xl md:text-8xl font-black font-display tracking-tighter leading-none">$500K</div>
            <p className="text-gray-400 font-black text-xl mt-4 font-display uppercase tracking-tight">Seed Round</p>
          </motion.div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-32">
          {funds.map((fund, i) => (
            <motion.div
              ...
              className="glass p-10 rounded-[2.5rem] border-white/5 hover:bg-white/[0.04] transition-all duration-500 h-full flex flex-col group"
            >
              ...
              <h4 className="text-white font-black text-xl mb-3 font-display tracking-tight leading-none">{fund.label}</h4>
              <p className="text-gray-500 text-base font-body leading-relaxed">{fund.desc}</p>
            </motion.div>
          ))}
        </div>

        <div className="relative glass p-12 md:p-20 rounded-[4rem] border-white/5 overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 relative z-10">
            {milestones.map((milestone, i) => (
              ...
              >
                ...
                <p className="text-gray-500 font-display font-black tracking-tight text-xs uppercase mb-2">Milestone {i+1}</p>
                <p className="text-white font-display font-black text-xl md:text-2xl tracking-tight leading-none">{milestone}</p>
              </motion.div>
            ))}
          </div>
        </div>
